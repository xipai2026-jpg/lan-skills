...
/plugin install blendcity@lan-skills
```

之后重启或 `/reload-plugins`，即可通过 `/jueling:jueling`、`/geo:geo`、`/frameshot:frameshot`、`/blendcity:blendcity` 或在对话中自然触发使用。
...
- **blendcity（20 分钟重建一座城）** — 给一个经纬度和半径，从 OpenStreetMap 拉真实建筑轮廓与楼高，用无头 Blender 建成 3D 体块城市并渲成图（鸟瞰/街区/俯视）。换城市只改中心点，地面与机位按数据范围自动算。成都天府广场 4229 栋建筑实测通过。纯标准库，不装 pip 包。
...
| `blendcity` | 代码 © Hugo Lan，可自由使用 | 两个脚本为自有代码，不含第三方代码。**但它消费的 OpenStreetMap 数据是 [ODbL 1.0](https://opendatacommons.org/licenses/odbl/)**：渲出的图属于 *Produced Work*，**可商用但须署名**「地理数据 © OpenStreetMap contributors (ODbL)」；若再分发 `scene.json` 这类加工后的数据集，则构成 Derivative Database，须按 ODbL 开放。 |
