...
### 🔴 `--virtual-time-budget` 管不住网络等待（别当 networkidle 用）
它只推进**虚拟时间**（定时器、动画、CSS transition），让「图片解码慢」这类等待快进。
**但它不覆盖真实网络等待。**

实测：模板引一个不可达外链，`--virtual-time-budget=2000`（2 秒）下**实际耗时 35.8 秒**；
加 `--offline` 后 **2.4 秒**。

所以两件事要分开：
- **图片/字体解码慢** → `--wait`（即 virtual-time-budget），默认 3000ms 够
- **外链够不着**（大陆机房够 Google Fonts、CDN 挂了、断网） → **`--offline`**

`--offline` 会让所有外部域名秒失败（`--host-resolver-rules="MAP * ~NOTFOUND"`）。
**样式和字体都在本地时建议常开** —— 外链只会拖死进程，不会让画面更对。
批量模式同样支持；另有 `--timeout`（单帧硬超时，默认 60 秒）兜底。

> ⚠️ 这个坑最坏的形态是**批量跑几十帧时 Chrome 进程常驻不退、零结果零报错**。
> 「批量 headless 渲染，先断网」当成默认习惯。
